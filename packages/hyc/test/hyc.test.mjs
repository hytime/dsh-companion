...
beforeAll(async () => {
  ...
  await mkdir(join(base, 'node_modules/@hytime/hyc-darwin-arm64/bin'), { recursive: true });
  await writeFile(join(base, 'node_modules/@hytime/hyc-darwin-arm64/package.json'),
    JSON.stringify({ name: '@hytime/hyc-darwin-arm64', version: '0.1.0' }));
  await writeFile(join(base, 'node_modules/@hytime/hyc-darwin-arm64/bin/hyc'),
    '#!/usr/bin/env node\nconsole.log("FAKE-HYC", process.argv.slice(2).join(" "));\n');
  await chmod(join(base, 'node_modules/@hytime/hyc-darwin-arm64/bin/hyc'), 0o755);
